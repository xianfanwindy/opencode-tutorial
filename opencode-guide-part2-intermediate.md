...
| 位置 | 优先级 | 适合场景 |
|------|--------|----------|
| `项目根目录/opencode.json` | 高 | 项目专用命令，提交到 Git 团队共享 |
| `~/.config/opencode/commands.json` | 低 | 个人通用命令，所有项目可用 |

项目级配置会与全局配置合并加载，同名命令以项目级为准。

---

## 6. 配置文件加载优先级

OpenCode 的配置来自多个来源，按优先级从高到低：

| 优先级 | 配置文件 | 说明 |
|--------|----------|------|
| 1（最高） | `项目根目录/opencode.json` | 项目专属配置，通常提交到 Git |
| 2 | `~/.config/opencode/opencode.json` | 用户全局配置 |
| 3 | `~/.config/opencode/commands.json` | 自定义命令（兼容旧版） |

**合并规则：** 高优先级数组会**覆盖**低优先级同名字段，而非合并。

### 重新加载配置

修改配置文件后，不用重启 OpenCode：

```bash
/reload
```

这会重新读取所有配置文件并生效。如果配置仍然不生效，确认文件路径是否正确：

```bash
opencode --verbose  # 启动时查看配置加载路径
```
...
> `${GITHUB_TOKEN}` 语法让 OpenCode 自动读取环境变量，Token 不会出现在配置文件中。
...
### 环境变量在 MCP 配置中不生效？
使用 `${VAR_NAME}` 语法而非 `$VAR_NAME`。确保变量在启动 OpenCode 的终端中已设置。

### 配置修改后不生效？
运行 `/reload` 重新加载所有配置文件。启动时加 `--verbose` 可查看配置加载路径。
